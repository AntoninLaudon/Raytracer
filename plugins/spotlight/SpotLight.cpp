...
Raytracer::Light::Light(const std::string name, Math::Point3D center, Math::Vector3D rotation, double radius, double radius2, PPM::RGB rgb)
{
    _name = name;
    _center = center;
    _rotation = rotation;
...
extern "C" Raytracer::IElement *createObject(Raytracer::Data data) {
    std::cout << "Creating Spot Light : " << data.getName() << std::endl;
    return new Raytracer::Light(data.getName(), data.getCenter(), data.getRotation(), data.getDouble(), data.getDouble2(), data.getRGB());
}

extern "C" Raytracer::ElemType getType() {
    return Raytracer::SPOTLIGHT;
}
