...
Raytracer::Sphere::Sphere(const std::string name, Math::Point3D center, Math::Vector3D rotation, Math::Vector3D direction, PPM::RGB rgb, double radius)
{
    _name = name;
    _center = center;
...
extern "C" Raytracer::IElement *createObject(Raytracer::Data data) {
    ...
    return new Raytracer::Sphere(data.getName(), data.getCenter(), data.getRotation(), data.getDirection(), data.getRGB(), data.getDouble());
}
